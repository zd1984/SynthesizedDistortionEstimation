...
#if ENABLE_VSDE
  /*free the depth error distribution*/
...
if (OccY[FINLVIEW] == NULL)
  ...
  zeroMeanWhiteNoiseInit(m_stdev, m_numberBins, m_distribution);
...
void processProbabilityMap(PROBABILITY_OUTCOME *node, int width, int height, bool debugFlag)
{
   if(node->numberOutcomes!=0)
   {
      ...
      if(numberNodesConnected!=node->numberOutcomes)
      {
  ...
  for (x = 0; x < Width2;  x++)
  {
    ...
    if (SplattingOption)
...
#if ENABLE_VSDE
      {
         ...
         centerProb = m_distribution[m_numberBins/2];

         for(i=0; i<m_numberBins; i++)
         {
            /*deal with center value at the end*/
            if(i!=(m_numberBins/2))
            {
               int newDepthVal = i - (m_numberBins/2) + (int)RefDepth[ViewId][ind];
               ...
               if((newDepthVal<0) || (newDepthVal>255))
               {
                  ...
                  centerProb = centerProb + m_distribution[i];
               }
               else
               {
                  ...
                  if( x2 < Width2 && x2 >= 0 )
                  {
                     addToProbabilityMap(&m_textureProbMap[ViewId].outcomes[y][x2], (unsigned char)newDepthVal, (unsigned short)x, (unsigned short)y, m_distribution[i]);
                  }
               }
            }
         }
         /*take care of the center value*/
         newz = 1.0 / ( (depthLevel/255.0) * (1/Znear[ViewId] - 1/Zfar[ViewId]) + (1/Zfar[ViewId]) );
         newdk = (FocalLength * LTranslation[ViewId] / newz) - duPrincipal[ViewId];
         x2 = (int)(floor( (x - newdk) * SubPelOption + 0.5));
         if( x2 < Width2 && x2 >= 0 )
         {
            addToProbabilityMap(&m_textureProbMap[ViewId].outcomes[y][x2], RefDepth[ViewId][y*Width2+x], (unsigned short)x, (unsigned short)y, centerProb);
         }
      }
#endif
  }
...
#if ENABLE_VSDE
void findMomentsAfterCombining(unsigned char*  refYLeft, unsigned char *refYRight, int width, int height, PROBABILITY_OUTCOME *left, PROBABILITY_OUTCOME *right, double factorLeft, double factorRight, PIXEL_MOMENTS *momentBuffer, int h, int w, bool debugFlag)
{
   int i;
   int j;

   double combinedFirst;
   double combinedSecond;
   ...
   momentBuffer->firstMoment[h*momentBuffer->width + w] = 0;
   momentBuffer->secondMoment[h*momentBuffer->width + w] = 0;

   /*left, the first and second moment excluding the case where it is a hole*/
   for(i=0; i<left->numberOutcomes; i++)
   {
	  leftFirst = leftFirst + ((double)refYLeft[left->y[i]*width + left->x[i]]) * left->prob[i];
	  leftSecond = leftSecond + ((double)refYLeft[left->y[i]*width + left->x[i]]) * ((double)refYLeft[left->y[i]*width + left->x[i]]) * left->prob[i];
   }

   /*right, the first and second moment excluding the case where it is a hole*/
   for(j=0; j<right->numberOutcomes; j++)
   {
	  rightFirst = rightFirst + ((double)refYRight[right->y[j]*width + right->x[j]]) * right->prob[j];
	  rightSecond = rightSecond + ((double)refYRight[right->y[j]*width + right->x[j]]) * ((double)refYRight[right->y[j]*width + right->x[j]]) * right->prob[j];
   }

   /*the case where both pixels contribute*/
   combinedFirst = factorLeft*leftFirst*(1 - right->noMapProb) + factorRight*rightFirst*(1 - left->noMapProb);
   combinedSecond = factorLeft*factorLeft*leftSecond*(1 - right->noMapProb) + factorRight*factorRight*rightSecond*(1 - left->noMapProb);
   combinedSecond = combinedSecond + 2*factorLeft*factorRight*leftFirst*rightFirst;
   momentBuffer->firstMoment[h*momentBuffer->width + w] = momentBuffer->firstMoment[h*momentBuffer->width + w] + combinedFirst;
   momentBuffer->secondMoment[h*momentBuffer->width + w] = momentBuffer->secondMoment[h*momentBuffer->width + w] + combinedSecond;

   /*the case where left pixel is a hole*/
   combinedFirst = rightFirst*left->noMapProb;
   combinedSecond = rightSecond*left->noMapProb;
   momentBuffer->firstMoment[h*momentBuffer->width + w] = momentBuffer->firstMoment[h*momentBuffer->width + w] + combinedFirst;
   momentBuffer->secondMoment[h*momentBuffer->width + w] = momentBuffer->secondMoment[h*momentBuffer->width + w] + combinedSecond;

   /*the case where right pixel is a hole*/
   combinedFirst = leftFirst*right->noMapProb;
   combinedSecond = leftSecond*right->noMapProb;
   momentBuffer->firstMoment[h*momentBuffer->width + w] = momentBuffer->firstMoment[h*momentBuffer->width + w] + combinedFirst;
   momentBuffer->secondMoment[h*momentBuffer->width + w] = momentBuffer->secondMoment[h*momentBuffer->width + w] + combinedSecond;

   /*the case where both pixels are holes*/
#if WANT_ZERO_FILL_INPAINTING
   combinedFirst = 128;
   combinedSecond =16384;
#else
   if((w-1)>=0)/*filling from left takes priority*/
   {
      combinedFirst = momentBuffer->firstMoment[h*momentBuffer->width + w-1];
      combinedSecond = momentBuffer->secondMoment[h*momentBuffer->width + w-1];
   }
   else if((h-1)>=0)
   {
      combinedFirst = momentBuffer->firstMoment[(h-1)*momentBuffer->width + w];
      combinedSecond = momentBuffer->secondMoment[(h-1)*momentBuffer->width + w];
   }
   else
   {
      combinedFirst = 128;
...
#endif
   /*scale and add*/
   momentBuffer->firstMoment[h*momentBuffer->width + w] = momentBuffer->firstMoment[h*momentBuffer->width + w] + combinedFirst*left->noMapProb*right->noMapProb;
   momentBuffer->secondMoment[h*momentBuffer->width + w] = momentBuffer->secondMoment[h*momentBuffer->width + w] + combinedSecond*left->noMapProb*right->noMapProb;
#if 0
   int i;
   int j;
   double leftNumber;
   ...
   for(i=0; i<left->numberOutcomes; i++)
   {
      for(j=0; j<right->numberOutcomes; j++)
      {
         ...
         leftNumber = (double)refYLeft[left->y[i]*width + left->x[i]];
         rightNumber = (double)refYRight[right->y[j]*width + right->x[j]];
   ...
   {
      rightNumber = (double)refYRight[right->y[i]*width + right->x[i]];
...
#if ENABLE_VSDE
  {
     ...
     for(i=0; i<Height; i++)
     {
        for(j=0; j<Width2; j++)
        {
           ...
           /*find the moments*/
           findMomentsAfterCombining(&RefView[LEFTVIEW][0], &RefView[RGHTVIEW][0], Width2, Height, &textureMapLeft->outcomes[i][j], &textureMapRight->outcomes[i][j], WeightLeft, WeightRight, &m_mergedPixelMoments, i, j, false);
