...
extern int myRand();
int generateWhiteNoise(int numberBins, double *distribution)
{
   int i;
   ...
   randNumber = ((double)myRand())/((double)MY_RAND_MAX);
 ...
 */
#if CORRUPT_DEPTH_MAP
bool CIYuv::ReadOneFrame(FILE *fp, int frameno, double *distribution, int numberBins, int enableFlag)
#else
...
#endif
{
  ...
  if(enableFlag)
  {
     ...
     for(i=0; i<height; i++)
     {
        ...
        {
           temp = (int)Y[i][j];
           temp = temp + generateWhiteNoise(numberBins, distribution);
