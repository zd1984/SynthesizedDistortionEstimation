...
#endif
#if WANT_ROPE_PMF
void createSharedMemoryNames(TCHAR output[256], char *input)
{
   int i=0;
   while(1)
   {
...
{
#if WANT_ROPE_PMF
  /*make sure the name of the shared memory is at the end*/
  TCHAR sharedMemName[256];
  createSharedMemoryNames(sharedMemName, argv[argc-1]);
  argc--;
#endif
  init_time();
#if MEMORY_DEBUG
  ...
  ROPEAlloc(p_Enc->p_Vid, 0.05f, sharedMemName);
#endif
  // encode sequence
...
#if WANT_ROPE_PMF
void ROPEcalculateFrameDistortion(VideoParameters *p_Vid)
{
   int i;
   int j;
   int k;
...
static void encode_sequence(VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  for (curr_frame_to_code = 0; curr_frame_to_code < frames_to_code; curr_frame_to_code++)
  {
    ...
    ROPEcalculateFrameDistortion(p_Vid);
