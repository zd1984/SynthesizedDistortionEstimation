...
#define USE_ZMAP     0      // Enable for now until the depth map can be generated with identical Znear and Zfar for the left and right views
...
#define VSRS3_ORIGINAL 1    // DT: This switch may be removed after London meeting.
...
#if ENABLE_VSDE
  /*shared pixel PMF memory*/
  HANDLE        textureMemLeft;
  HANDLE        textureMemRight;
  HANDLE        depthMemLeft;
  HANDLE        depthMemRight;
  PixelPMF      *texturePMF1D[2]; /*1D shared texture memory buffer*/
  PixelPMF      *depthPMF1D[2];   /*1D shared depth memory buffer*/
  PixelPMF      **texturePMF[2];  /*2D access texture buffers for left and right*/
  PixelPMF      **depthPMF[2];    /*2D access depth buffers for left and right*/
  PROBABILITY_MAP m_textureProbMap[2];              /*texture distribution buffer for left and right view*/
  PIXEL_MOMENTS m_mergedPixelMoments;               /*moments for the final merged view*/
#endif
...
#if ENABLE_VSDE
  void setSharedMemoryHandle(HANDLE tLeft, HANDLE tRight, HANDLE dLeft, HANDLE dRight) {textureMemLeft = tLeft; textureMemRight = tRight; depthMemLeft = dLeft; depthMemRight = dRight;}
