...
#define SAVE_ENCODER_LUMA_RECON       0
#define SAVE_MANUAL_LUMA_RECON        0
#define WANT_ENC_INFO_EXTRACTION      0
#define WANT_ROPE_PMF                 1
...
#if WANT_ROPE_PMF
/*We like to remove the printf, don't mind the compiler warning for now*/
//#define emptyLog()
//#define printf emptyLog
#endif
...
typedef struct video_par
{
  ...
  //end;
#if WANT_ROPE_PMF
  HANDLE mailHandleOut;
  HANDLE mailHandleIn;
  float lossRate;
  HANDLE hMapFile;              //handle used for shared memory
  PixelPMF *pixelDistribution1DShared;//a consecutive piece of memory as shared memory
  PixelPMF **pixelDistributionShared;//2D array for easy of access
  PixelPMF *pixelDistribution1DWorking;//frame level working memory
  PixelPMF **pixelDistributionWorking;//2D array for easy of access
  int **frameResidue;
  imgpel **framePrediction; //don't really need the prediction for ROPE but we extract it to test reconstruction to make sure everything is correct
  imgpel best8x8Pred[16][16];//it is temporary and updated on a MB basis
  int best8x8Res[16][16];//it is temporary and updated on a MB basis
#endif
} VideoParameters;
