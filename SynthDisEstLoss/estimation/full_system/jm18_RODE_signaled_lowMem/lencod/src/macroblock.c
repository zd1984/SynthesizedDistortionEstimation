...
#if WANT_ROPE_PMF
    /*another case of coefficient truncation*/
	for(i=0; i<8; i++)
	{
		for(j=0; j<8; j++)
		{
			p_Vid->frameResidue[currMB->pix_y + ((block8x8/2)*8) + i][currMB->pix_x + ((block8x8%2)*8) + j] = 0;
			p_Vid->framePrediction[currMB->pix_y + ((block8x8/2)*8) + i][currMB->pix_x + ((block8x8%2)*8) + j] = currSlice->mb_pred[0][((block8x8/2)*8) + i][((block8x8%2)*8) + j];
		}
	}
#endif
  }
...
#if WANT_ROPE_PMF //keep it here as a assertion
        else
        {
			exit(0);
		}
...
void luma_residual_coding (Macroblock *currMB)
{
  ...
  if (is_skip)
  {
	...
	for(i=0; i<16; i++)
	{
		for(j=0; j<16; j++)
		{
			p_Vid->frameResidue[currMB->pix_y + i][currMB->pix_x + j] = 0;
			p_Vid->framePrediction[currMB->pix_y + i][currMB->pix_x + j] = currSlice->mb_pred[0][i][j];
		}
	}
#endif
  }
...
#if WANT_ROPE_PMF
    /*if coefficient truncation happens we zero*/
	for(i=0; i<16; i++)
	{
		for(j=0; j<16; j++)
		{
