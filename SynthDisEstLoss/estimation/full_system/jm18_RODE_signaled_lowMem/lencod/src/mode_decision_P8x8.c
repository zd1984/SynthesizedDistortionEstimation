                                     ...
                                     int transform8x8)
{
  ...
  if (valid_8x8 == TRUE)
  {
    ...
    for(i=0; i<8; i++)
    {
		for(j=0; j<8; j++)
		{
			p_Vid->best8x8Res[((block/2)*8) + i][((block%2)*8) + j] = p_Vid->frameResidue[currMB->pix_y + ((block/2)*8) + i][currMB->pix_x + ((block%2)*8) + j];
			p_Vid->best8x8Pred[((block/2)*8) + i][((block%2)*8) + j] = p_Vid->framePrediction[currMB->pix_y + ((block/2)*8) + i][currMB->pix_x + ((block%2)*8) + j];
		}
	}
#endif
    //--- store reconstruction and prediction ---
...
#if WANT_ROPE_PMF //we keep this assertion here to make sure that reconstruction are all done by currSlice->luma_residual_coding_8x8
  else
  {
	  exit(0);
  }
