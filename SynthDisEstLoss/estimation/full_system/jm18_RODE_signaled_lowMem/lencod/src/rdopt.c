      ...
      copy_image_data_16x16(&p_Vid->enc_picture->imgY[currMB->pix_y], p_RDO->tr4x4->mpr8x8, currMB->pix_x, 0);
		...
		for(i=0; i<16; i++)
		{
			for(j=0; j<16; j++)
			{
				...
				p_Vid->framePrediction[currMB->pix_y + i][currMB->pix_x + j] = p_RDO->tr4x4->mpr8x8[i][j];
			}
		}
#endif
      if(p_Inp->rdopt == 3)
...
#if WANT_ROPE_PMF
		/*restore the prediction and residue*/
		for(i=0; i<16; i++)
		{
			for(j=0; j<16; j++)
			{
				p_Vid->frameResidue[currMB->pix_y + i][currMB->pix_x + j] = p_Vid->best8x8Res[i][j];
				p_Vid->framePrediction[currMB->pix_y + i][currMB->pix_x + j] = p_Vid->best8x8Pred[i][j];
			}
		}
#endif
      if(p_Inp->rdopt == 3)
