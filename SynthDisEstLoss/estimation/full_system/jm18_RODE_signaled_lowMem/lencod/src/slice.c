...
#if WANT_ROPE_PMF
/*only used by the first I frame*/
void ROPEAddToPMFFirstFrame(float prob, int val, PixelPMF *distribution)
{
   if(distribution->map[val]!=0)
   {
      printf("Error %s \n", __FUNCTION__);
      exit(0);
   }
   ...
   {
      distribution->map[val] = 1;
      distribution->prob[val] = prob;
   }
}
...
void ROPEMergeTwoPMF(PixelPMF *src1, int shift1, float factor1, PixelPMF *src2, int shift2, float factor2, PixelPMF *dst)
{
   int i;
   ...
   memset(tempProbA, 0, sizeof(float)*256);
   memset(tempProbB, 0, sizeof(float)*256);
   memset(tempProbC, 0, sizeof(float)*256);
   ...
   for(i=0; i<256; i++)
   {
      if(src1->map[i]!=0)
      {
         ...
         tempMapA[index]=1;
         tempProbA[index]=tempProbA[index]+src1->prob[i];
      }
   }
   ...
   for(i=0; i<256; i++)
   {
      if(src2->map[i]!=0)
      {
         ...
         tempMapB[index]=1;
         tempProbB[index]=tempProbB[index]+src2->prob[i];
      }
   }
   ...
   for(i=0; i<256; i++)
   {
      ...
      if(tempMapC[i]!=0)
      {
         tempMapC[i] = 1; //to prevent overflow set it back to 1
         tempProbC[i] = tempProbA[i]*factor1 + tempProbB[i]*factor2;
      }
   }

   memcpy(dst->prob, tempProbC, sizeof(float)*256);
   memcpy(dst->map, tempMapC, sizeof(unsigned char)*256);
}
void ROPEupdateISliceMBPMF(VideoParameters *p_Vid, Macroblock *currMB)
{
   ...
   if(p_Vid->frame_no == 0)
   {
      for(i=0; i<16; i++)
      {
         for(j=0; j<16; j++)
         {
            ...
            ROPEAddToPMFFirstFrame(1, currentReconstructedPixel, &p_Vid->pixelDistributionWorking[currMB->pix_y + i][currMB->pix_x + j]);
         }
      }
   }
   else
   {
      for(i=0; i<16; i++)
      {
         for(j=0; j<16; j++)
         {
            PixelPMF tempDistribution;
            int currentReconstructedPixel = (int)((unsigned int)img_enc[currMB->pix_y + i][currMB->pix_x + j]);

            memset(&tempDistribution.prob, 0, sizeof(float)*256);
            memset(&tempDistribution.map, 0, sizeof(unsigned char)*256);
            tempDistribution.map[currentReconstructedPixel] = 1;
            tempDistribution.prob[currentReconstructedPixel] = 1;

            ROPEMergeTwoPMF(&p_Vid->pixelDistributionShared[currMB->pix_y + i][currMB->pix_x + j], 0, p_Vid->lossRate, &tempDistribution, 0, 1 - p_Vid->lossRate, &p_Vid->pixelDistributionWorking[currMB->pix_y + i][currMB->pix_x + j]);
         }
      }
   }
}
...
#define ILUMAPADY 12
int CheckVertMV(VideoParameters *p_Vid, Macroblock *currMB, int vec1_y, int block_size_y)
{
   int y_pos = vec1_y>>2;
   int maxold_y = p_Vid->height - 1;

   if(y_pos < (-ILUMAPADY + 2) || y_pos > (maxold_y + ILUMAPADY - block_size_y - 2))
   {
      return 1;
   }
   else
...
void ROPEMergeTwoBlocks(VideoParameters *p_Vid, Macroblock *currMB, int blockOffsetX, int blockOffsetY, int x_pos, int y_pos, int hor_block_size, int ver_block_size)
{
   int i;
   int j;
   ...
   x_pos >>= 2;
   y_pos >>= 2;
   x_pos = iClip3(-18, p_Vid->width - 1 + 2, x_pos);
   y_pos = iClip3(-10, p_Vid->height - 1 + 2, y_pos);


   for(i=0; i<ver_block_size; i++)
   {
      for(j=0; j<hor_block_size; j++)
      {
         PixelPMF *predPixelPMF;
         PixelPMF *concealingPixelPMF;
         PixelPMF *targetPixelPMF;

         /*do an error checking first*/
         if( (y_pos + i < -IMG_PAD_SIZE_Y) || (y_pos + i >= (p_Vid->height+IMG_PAD_SIZE_Y)))
         {
            printf("Error:MV points to offscreen area(Y) y:%d x:%d i:%d j:%d frame %d\n", currMB->pix_y, currMB->pix_x, i, j, p_Vid->frame_no);
            exit(0);
         }

         if( (x_pos + j < -IMG_PAD_SIZE_X) || (x_pos + j >= (p_Vid->width+IMG_PAD_SIZE_X)))
         {
            printf("Error:MV points to offscreen area(X) y:%d x:%d i:%d j:%d frame %d\n", currMB->pix_y, currMB->pix_x, i, j, p_Vid->frame_no);
            exit(0);
         }

         predPixelPMF = &p_Vid->pixelDistributionShared[y_pos + i][x_pos + j];
         concealingPixelPMF = &p_Vid->pixelDistributionShared[currMB->pix_y + blockOffsetY*4 + i][currMB->pix_x + blockOffsetX*4 + j];
         targetPixelPMF = &p_Vid->pixelDistributionWorking[currMB->pix_y + blockOffsetY*4 + i][currMB->pix_x + blockOffsetX*4 + j];

         ROPEMergeTwoPMF(concealingPixelPMF, 0, p_Vid->lossRate, predPixelPMF, rshift_rnd_sf(p_Vid->frameResidue[currMB->pix_y + blockOffsetY*4 + i][currMB->pix_x + blockOffsetX*4 + j], DQ_BITS), 1 - p_Vid->lossRate, targetPixelPMF);
      }
   }
}
/*This function follows what perform_mc_single and get_block_luma do in terms of finding the MB prediction*/
void ROPEprocessBlockPMF(VideoParameters *p_Vid, Macroblock *currMB, int blockOffsetX, int blockOffsetY, int mvX, int mvY, int block_size_x, int block_size_y)
{
   int dest_x, dest_y;

   if(((mvX%4)!=0) || ((mvY%4)!=0) )
   {
      printf("ROPE does not support sub-pel ME yet. Disable sub-pel ME. mvX:%d mvY:%d at %d %d frame %d\n", mvX, mvY, (currMB->block_x + blockOffsetX)*4, (currMB->block_y + blockOffsetY)*4, p_Vid->frame_no);
      exit(0);
   }
   dest_x = (currMB->block_x + blockOffsetX) * 16 + mvX;
   dest_y = (currMB->block_y + blockOffsetY) * 16 + mvY;

   if(block_size_y > (ILUMAPADY-4) && CheckVertMV(p_Vid, currMB, dest_y, block_size_y))
   {
      ROPEMergeTwoBlocks(p_Vid, currMB, blockOffsetX, blockOffsetY, dest_x, dest_y, block_size_x, BLOCK_SIZE_8x8);
      ROPEMergeTwoBlocks(p_Vid, currMB, blockOffsetX, blockOffsetY+(BLOCK_SIZE_8x8>>2), dest_x, dest_y + BLOCK_SIZE_8x8_SP, block_size_x, block_size_y-BLOCK_SIZE_8x8);
   }
   else
   {
      ROPEMergeTwoBlocks(p_Vid, currMB, blockOffsetX, blockOffsetY, dest_x, dest_y, block_size_x, block_size_y);
   }
}
unsigned char block_scan[16] = {0, 1, 4, 5, 2, 3, 6, 7, 8, 9, 12, 13, 10, 11, 14, 15};
void ROPEupdatePSliceMBPMF(VideoParameters *p_Vid, Macroblock *currMB)
{
   ...
   if( (currMB->mb_type == P16x16) || (currMB->mb_type==PSKIP) )
   {
      mv_x =  motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_x;
      mv_y =  motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_y;
      ROPEprocessBlockPMF(p_Vid, currMB, 0, 0, mv_x, mv_y, 16, 16);
   }
   else if(currMB->mb_type == P16x8)
   {
      mv_x =  motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_x;
      mv_y =  motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_y;
      ROPEprocessBlockPMF(p_Vid, currMB, 0, 0, mv_x, mv_y, 16, 8);

      mv_x =  motion[currMB->block_y + 2][currMB->block_x + 0].mv[LIST_0].mv_x;
      mv_y =  motion[currMB->block_y + 2][currMB->block_x + 0].mv[LIST_0].mv_y;
      ROPEprocessBlockPMF(p_Vid, currMB, 0, 2, mv_x, mv_y, 16, 8);
   }
   else if(currMB->mb_type == P8x16)
   {
      mv_x =  motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_x;
      mv_y =  motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_y;
      ROPEprocessBlockPMF(p_Vid, currMB, 0, 0, mv_x, mv_y, 8, 16);

      mv_x =  motion[currMB->block_y + 0][currMB->block_x + 2].mv[LIST_0].mv_x;
      mv_y =  motion[currMB->block_y + 0][currMB->block_x + 2].mv[LIST_0].mv_y;
      ROPEprocessBlockPMF(p_Vid, currMB, 2, 0, mv_x, mv_y, 8, 16);
   }
   else if(currMB->mb_type == P8x8)
   {
      ...
      {
         int k;
         int mv_mode  = currMB->b8x8[block8x8].mode;
         ...
         {
            int block_x =  (block_scan[k] & 3);
            int block_y = ((block_scan[k] >> 2) & 3);

            mv_x =  motion[currMB->block_y + block_y][currMB->block_x + block_x].mv[LIST_0].mv_x;
            mv_y =  motion[currMB->block_y + block_y][currMB->block_x + block_x].mv[LIST_0].mv_y;
            ROPEprocessBlockPMF(p_Vid, currMB, block_x, block_y, mv_x, mv_y, block_size_x, block_size_y);
         }
      }
   }
   else
   {
      printf("Error %s mode not recognized\n", __FUNCTION__, currMB->mb_type);
      exit(0);
   }
}
#endif
...
int encode_one_slice (VideoParameters *p_Vid, int SliceGroupId, int TotalCodedMBs)
{
  ...
  else if(currMB->mb_type==IPCM)
  {
    ...
    /*printf mode and mv here*/
    {
      ...
      {
         printf("P_8_8:");
         printf("%d %d(%d),", motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_x, motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_y, currMB->b8x8[0].mode);
         printf("%d %d(%d),", motion[currMB->block_y + 0][currMB->block_x + 2].mv[LIST_0].mv_x, motion[currMB->block_y + 0][currMB->block_x + 2].mv[LIST_0].mv_y, currMB->b8x8[1].mode);
         printf("%d %d(%d),", motion[currMB->block_y + 2][currMB->block_x + 0].mv[LIST_0].mv_x, motion[currMB->block_y + 2][currMB->block_x + 0].mv[LIST_0].mv_y, currMB->b8x8[2].mode);
         printf("%d %d(%d)*", motion[currMB->block_y + 2][currMB->block_x + 2].mv[LIST_0].mv_x, motion[currMB->block_y + 2][currMB->block_x + 2].mv[LIST_0].mv_y, currMB->b8x8[3].mode);
      }
...
#endif
#if WANT_ROPE_PMF
   if(currMB->mb_type==I16MB || currMB->mb_type==I8MB || currMB->mb_type==I4MB)
   {
      ROPEupdateISliceMBPMF(p_Vid, currMB);
   }
   else if(currMB->mb_type==PSKIP || currMB->mb_type==P16x16 || currMB->mb_type==P16x8 || currMB->mb_type==P8x16 || currMB->mb_type==P8x8)
   {
      ROPEupdatePSliceMBPMF(p_Vid, currMB);
   }
   else
   {
      printf("Other type encountered.Go check!\n");
      exit(0);
   }
#endif
  }
