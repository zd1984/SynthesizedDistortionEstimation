...
#if WANT_ROPE_PMF
void ROPEAddToPMF(double prob, int val, PixelPMF *distribution)
{
   if(distribution->count == PMF_ALLOCATION_SLOTS)
   {
      double *newProb;
      int *newVal;
      distribution->capacity = distribution->capacity + PMF_ALLOCATION_SLOTS;
      newProb = (double *)malloc(sizeof(double)*distribution->capacity);
      newVal = (int *)malloc(sizeof(int)*distribution->capacity);

      memcpy(newProb, distribution->prob, sizeof(double)*distribution->count);
      memcpy(newVal, distribution->val, sizeof(int)*distribution->count);
      newProb[distribution->count] = prob;
      newVal[distribution->count] = val;
      distribution->count++;

      free(distribution->prob);
      free(distribution->val);

      distribution->prob = newProb;
      distribution->val = newVal;
   }
   else
   {
      distribution->prob[distribution->count] = prob;
      distribution->val[distribution->count] = val;
      distribution->count++;
   }
}
/*once the capacity increases, the old entries are gone*/
void ROPEIncreasePMFCapacity(PixelPMF *distribution, int intendedSize)
{
   double *newProb;
   int *newVal;
   int newCapacity;

   newCapacity = ((intendedSize/PMF_ALLOCATION_SLOTS)+1)*PMF_ALLOCATION_SLOTS;

   distribution->capacity = newCapacity;//distribution->capacity + PMF_ALLOCATION_SLOTS;
   newProb = (double *)malloc(sizeof(double)*distribution->capacity);
   newVal = (int *)malloc(sizeof(int)*distribution->capacity);
   if((newProb==NULL) || (newVal==NULL))
   {
	   printf("Error %s allocating memory\n",__FUNCTION__);
	   exit(0);
   }

   /*memcpy(newProb, distribution->prob, sizeof(double)*distribution->count);
   memcpy(newVal, distribution->val, sizeof(int)*distribution->count);*/
   if((distribution->prob==NULL) || (distribution->val==NULL))
   {
	   printf("Error %s freeing memory\n",__FUNCTION__);
	   exit(0);
   }
   free(distribution->prob);
   free(distribution->val);
...
void ROPEcopyDistribution(PixelPMF *destination, PixelPMF *source)
{
   memcpy(destination->prob, source->prob, sizeof(double)*source->count);
   memcpy(destination->val, source->val, sizeof(int)*source->count);
...
void ROPEprocessPMFPaddedRegion(VideoParameters *p_Vid)
{
	...
	{
		dst = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][-IMG_PAD_SIZE_Y][i];
		src = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][0][0];
		/*while(src->count > dst->capacity)
		{
			ROPEIncreasePMFCapacity(dst);
		}*/
		ROPEIncreasePMFCapacity(dst, src->count);
		ROPEcopyDistribution(dst, src);
	}
	/*first row, middle region*/
	for (i = 0; i < p_Vid->width; i++)
	{
		dst = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][-IMG_PAD_SIZE_Y][i];
		src = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][0][i];
		/*while(src->count > dst->capacity)
		{
			ROPEIncreasePMFCapacity(dst);
		}*/
	...
	{
		dst = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][-IMG_PAD_SIZE_Y][i];
		src = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][0][p_Vid->width-1];
		/*while(src->count > dst->capacity)
		{
			ROPEIncreasePMFCapacity(dst);
		}*/
	...
	for(i=-IMG_PAD_SIZE_Y+1; i<0; i++)
	{
		...
		{
			dst = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][i][j];
			src = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][-IMG_PAD_SIZE_Y][j];
	...
	for(i=0; i<p_Vid->height; i++)
	{
		...
		{
			dst = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][i][j];
			src = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][i][0];
	...
	for(i=0; i<p_Vid->height; i++)
	{
		...
		{
			dst = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][i][j];
			src = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][i][p_Vid->width-1];
	...
	for (i = -IMG_PAD_SIZE_X; i < 0; i++)
	{
		dst = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][p_Vid->height][i];
		src = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][p_Vid->height-1][0];
		/*while(src->count > dst->capacity)
		{
			ROPEIncreasePMFCapacity(dst);
		}*/
	...
	{
		dst = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][p_Vid->height][i];
		src = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][p_Vid->height-1][i];
		/*while(src->count > dst->capacity)
		{
			ROPEIncreasePMFCapacity(dst);
		}*/
	...
	{
		dst = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][p_Vid->height][i];
		src = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][p_Vid->height-1][p_Vid->width-1];
		/*while(src->count > dst->capacity)
		{
			ROPEIncreasePMFCapacity(dst);
		}*/
	...
	for(i=p_Vid->height+1; i<p_Vid->height+IMG_PAD_SIZE_Y; i++)
	{
		...
		{
			dst = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][i][j];
			src = &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][p_Vid->height][j];
...
void ROPEupdateISliceMBPMF(VideoParameters *p_Vid, Macroblock *currMB)
{
   int i;
   int j;
   int k;
   int lastFrameIndex = (p_Vid->currentFrameIndex+1)%2;
   Slice *currSlice = currMB->p_Slice;
   imgpel **img_enc = p_Vid->enc_picture->p_curr_img; /*reconstructed image*/

   /*a special case for the first frame, we have to assume there is no loss in the first frame or the first frame is concealed in a certain way*/
   ...
   if(p_Vid->frame_no == 0)
   {
      for(i=0; i<16; i++)
      {
         for(j=0; j<16; j++)
         {
            int currentReconstructedPixel = (int)((unsigned int)img_enc[currMB->pix_y + i][currMB->pix_x + j]);

            ROPEAddToPMF(1, currentReconstructedPixel, &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j]);
         }
      }
   }
   else
   {
      for(i=0; i<16; i++)
      {
         for(j=0; j<16; j++)
         {
            ...
            /*copy the distribution of the co-located pixel to the current pixel*/
            /*while(p_Vid->pixelDistribution[lastFrameIndex][currMB->pix_y + i][currMB->pix_x + j].count > p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j].capacity)
            {
               ROPEIncreasePMFCapacity(&p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j]);
            }*/
            ROPEIncreasePMFCapacity(&p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j], p_Vid->pixelDistribution[lastFrameIndex][currMB->pix_y + i][currMB->pix_x + j].count);
            ROPEcopyDistribution(&p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j], &p_Vid->pixelDistribution[lastFrameIndex][currMB->pix_y + i][currMB->pix_x + j]);


            for(k=0; k<p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j].count; k++)
            {
               p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j].prob[k] = p_Vid->lossRate * p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j].prob[k];
               if(currentReconstructedPixel==p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j].val[k])
               {
                  p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j].prob[k] = p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j].prob[k] + (1 - p_Vid->lossRate);
                  found = 1;
               }
            }
            if(found == 0)
            {
               ROPEAddToPMF(1 - p_Vid->lossRate, currentReconstructedPixel, &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j]);
            }
         }
      }
   }
...
double tempProbA[256];
unsigned char tempMapA[256];
double tempProbB[256];
unsigned char tempMapB[256];
double tempProbC[256];
unsigned char tempMapC[256];
void ROPEMergeTwoPMF(PixelPMF *src1, int shift1, double factor1, PixelPMF *src2, int shift2, double factor2, PixelPMF *dst)
{
	int i;
	int count=0;

	memset(tempMapA, 0, sizeof(unsigned char)*256);
	memset(tempMapB, 0, sizeof(unsigned char)*256);
	memset(tempMapC, 0, sizeof(unsigned char)*256);
	memset(tempProbA, 0, sizeof(double)*256);
	memset(tempProbB, 0, sizeof(double)*256);
	memset(tempProbC, 0, sizeof(double)*256);

	for(i=0; i<src1->count; i++)
	{
		int index = src1->val[i]+shift1;
		...
		tempMapA[index]=1;
		tempProbA[index]=src1->prob[i];
	}

	for(i=0; i<src2->count; i++)
	{
		int index = src2->val[i]+shift2;
		if(index<0)
		{
			index=0;
		}
		...
		tempMapB[index]=1;
		tempProbB[index]=src2->prob[i];
	}

	for(i=0; i<256; i++)
	{
		tempMapC[i] = tempMapA[i] + tempMapB[i];
		if(tempMapC[i]!=0)
		{
			tempProbC[i] = tempProbA[i]*factor1 + tempProbB[i]*factor2;
			count++;
		}
	}

	/*while(count > dst->capacity)
	{
		ROPEIncreasePMFCapacity(dst);
	}*/
	ROPEIncreasePMFCapacity(dst, count);
	for(i=0; i<256; i++)
	{
		if(tempMapC[i]!=0)
		{
			dst->val[dst->count] = i;
			dst->prob[dst->count] = tempProbC[i];
			dst->count++;
		}
	}
...
void ROPEupdatePSliceMBPMF(VideoParameters *p_Vid, Macroblock *currMB)
{
   int i;
   int j;
   ...
   for(i=0; i<16; i++)
   {
	   for(j=0; j<16; j++)
	   {
		   ...
		   int mcIndex;
			...
			/*find the location of the pixel corresponding to this pixel*/
			if(motion[currMB->block_y + i/4][currMB->block_x + j/4].mv[LIST_0].mv_x%4 != 0)
			{
				printf("ROPE does not support sub-pel ME yet. Disable sub-pel ME\n");
				exit(0);
			}
			if(motion[currMB->block_y + i/4][currMB->block_x + j/4].mv[LIST_0].mv_y%4 != 0)
			{
				printf("ROPE does not support sub-pel ME yet. Disable sub-pel ME\n");
				exit(0);
			}
			mv_x =  motion[currMB->block_y + i/4][currMB->block_x + j/4].mv[LIST_0].mv_x>>2;
			mv_y =  motion[currMB->block_y + i/4][currMB->block_x + j/4].mv[LIST_0].mv_y>>2;

			/*Grab the distribution of the pixel pointed by the mv in the last frame*/
			/*do a check first to make sure the mv doesn't point to the off screen area*/
			if( ((currMB->pix_y + i + mv_y) < -IMG_PAD_SIZE_Y) || ((currMB->pix_y + i + mv_y) >= (p_Vid->height+IMG_PAD_SIZE_Y)))
			{
				printf("Error:MV points to offscreen area(Y) y:%d x:%d i:%d j:%d mv_y:%d mv_x:%d\n", currMB->pix_y, currMB->pix_x, i, j, mv_y, mv_x);
				exit(0);
			}
			if( ((currMB->pix_x + j + mv_x) < -IMG_PAD_SIZE_X) || ((currMB->pix_x + j + mv_x) >= (p_Vid->width+IMG_PAD_SIZE_X)))
			{
				printf("Error:MV points to offscreen area(X) y:%d x:%d i:%d j:%d mv_y:%d mv_x:%d\n", currMB->pix_y, currMB->pix_x, i, j, mv_y, mv_x);
				exit(0);
			}
			predPixelPMF = &p_Vid->pixelDistribution[lastFrameIndex][currMB->pix_y + i + mv_y][currMB->pix_x + j + mv_x];

			ROPEMergeTwoPMF(&p_Vid->pixelDistribution[lastFrameIndex][currMB->pix_y + i][currMB->pix_x + j], 0, p_Vid->lossRate, predPixelPMF, rshift_rnd_sf(p_Vid->frameResidue[currMB->pix_y + i][currMB->pix_x + j], DQ_BITS), 1 - p_Vid->lossRate, &p_Vid->pixelDistribution[p_Vid->currentFrameIndex][currMB->pix_y + i][currMB->pix_x + j]);
	   }
   }
   //printf("P Frame:%d\n",p_Vid->frame_no);
}
...
extern unsigned char manReconData[1088][1920]; //make it big enough for 1080p
void saveManRecon(VideoParameters *p_Vid, Macroblock *currMB)//debug
{
   int i;
   int j;
   ...
   for(i=0; i<16; i++)
   {
      for(j=0; j<16; j++)
	  {
	     manReconData[currMB->pix_y + i][currMB->pix_x + j] = (unsigned char)iClip1(255, p_Vid->framePrediction[currMB->pix_y + i][currMB->pix_x + j]+rshift_rnd_sf(p_Vid->frameResidue[currMB->pix_y + i][currMB->pix_x + j], DQ_BITS));
	  }
   }

}//debug
void saveImgY(VideoParameters *p_Vid, Macroblock *currMB)//debug
{
	int i;
	int j;
	...
	imgpel **img_enc;
	   ...
	   img_enc = p_Vid->enc_picture->imgY; /*reconstructed image y*/
	   for(i=0; i<16; i++)
	   {
		   for(j=0; j<16; j++)
		   {
			   manReconData[currMB->pix_y + i][currMB->pix_x + j] = (unsigned char)img_enc[currMB->pix_y + i][currMB->pix_x + j];
		   }
	   }
}//debug
#endif
...
int encode_one_slice (VideoParameters *p_Vid, int SliceGroupId, int TotalCodedMBs)
{
  ...
  else if(currMB->mb_type==IPCM)
  {
...
#if WANT_ENC_INFO_EXTRACTION
    /*printf mode and mv here*/
    {
      PicMotionParams **motion = p_Vid->enc_picture->mv_info;
      if(currMB->mb_type==PSKIP)
      {
         printf("P_SKIP:");
         printf("%d %d*", motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_x, motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_y);
      }
      ...
      if(currMB->mb_type==P16x8)
      {
         printf("P_16_8:");
         printf("%d %d,", motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_x, motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_y);
         printf("%d %d*", motion[currMB->block_y + 2][currMB->block_x + 0].mv[LIST_0].mv_x, motion[currMB->block_y + 2][currMB->block_x + 0].mv[LIST_0].mv_y);
      }
      if(currMB->mb_type==P8x16)
      {
         printf("P_8_16:");
         printf("%d %d,", motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_x, motion[currMB->block_y + 0][currMB->block_x + 0].mv[LIST_0].mv_y);
         printf("%d %d*", motion[currMB->block_y + 0][currMB->block_x + 2].mv[LIST_0].mv_x, motion[currMB->block_y + 0][currMB->block_x + 2].mv[LIST_0].mv_y);
      }
      if(currMB->mb_type==P8x8)
      {
         printf("P_8_8:");
         ...
         printf("%d %d,", motion[currMB->block_y + 0][currMB->block_x + 2].mv[LIST_0].mv_x, motion[currMB->block_y + 0][currMB->block_x + 2].mv[LIST_0].mv_y);
         printf("%d %d,", motion[currMB->block_y + 2][currMB->block_x + 0].mv[LIST_0].mv_x, motion[currMB->block_y + 2][currMB->block_x + 0].mv[LIST_0].mv_y);
         printf("%d %d*", motion[currMB->block_y + 2][currMB->block_x + 2].mv[LIST_0].mv_x, motion[currMB->block_y + 2][currMB->block_x + 2].mv[LIST_0].mv_y);
      }
      ...
      if(currMB->p_Slice->slice_type == P_SLICE)
      {
         if(currMB->mb_type==I16MB)
         {
            printf("P_I16*");
      ...
      if(currMB->mb_x == ((p_Vid->enc_picture->size_x>>4) - 1))
      {
         printf("\n");
      }
      if( (currMB->mb_y == ((p_Vid->enc_picture->size_y>>4) - 1)) && (currMB->mb_x == ((p_Vid->enc_picture->size_x>>4) - 1)) )
      {
         printf("\n");
      }
   }
   ...
   if(currMB->p_Slice->slice_type == I_SLICE)
   {
      ROPEupdateISliceMBPMF(p_Vid, currMB);
   }
   if(currMB->p_Slice->slice_type == P_SLICE)
   {
      ROPEupdatePSliceMBPMF(p_Vid, currMB);
   }
   if(p_Vid->frame_no > 0)
   {
