 ...
 */
#if WANT_SLICE_DROP
#define SAVE_LAST_FRAME 0
#if SAVE_LAST_FRAME
FILE *outFile;
#endif
StorablePicture last_frame_copy;
void freeLastFrame()
{
   if( (last_frame_copy.imgY!=NULL) && (last_frame_copy.imgUV!=NULL) )
   {
      /*normal*/
   }
   else
   {
      printf("freeLastFrame frame buffers not properly allocated\n");
   }

   free_mem2Dpel_pad(last_frame_copy.imgY, MCBUF_LUMA_PAD_Y, MCBUF_LUMA_PAD_X);
   free_mem3Dpel_pad(last_frame_copy.imgUV, MCBUF_CHROMA_PAD_Y, MCBUF_CHROMA_PAD_X);
#if SAVE_LAST_FRAME
   fclose(outFile);
#endif
}
void clearLastFrame()
{
   memset(&last_frame_copy, 0, sizeof(StorablePicture));
}
static void copyFromLastFrame(VideoParameters *p_Vid)
{
   int i;
   ...
   {
      memcpy(&(p_Vid->dec_picture->imgY[i][0]), &(last_frame_copy.imgY[i][0]), p_Vid->width*sizeof(imgpel));
   }

   for(i=0; i<p_Vid->height/2; i++)
   {
      memcpy(&(p_Vid->dec_picture->imgUV[0][i][0]), &(last_frame_copy.imgUV[0][i][0]), p_Vid->width*sizeof(imgpel)/2);
      memcpy(&(p_Vid->dec_picture->imgUV[1][i][0]), &(last_frame_copy.imgUV[1][i][0]), p_Vid->width*sizeof(imgpel)/2);
   }
}
static void copyToLastFrame(VideoParameters *p_Vid, StorablePicture *dec_picture)
{
   int i;
   int j;
   ...
   {
      /*allocate*/
      get_mem2Dpel_pad(&(last_frame_copy.imgY), p_Vid->height, p_Vid->width, MCBUF_LUMA_PAD_Y, MCBUF_LUMA_PAD_X);
      get_mem3Dpel_pad(&(last_frame_copy.imgUV), 2, p_Vid->height/2, p_Vid->width/2, MCBUF_CHROMA_PAD_Y, MCBUF_CHROMA_PAD_X);
#if SAVE_LAST_FRAME
      outFile = fopen("lastFrame.cif","wb");
      if(outFile == NULL)
      {
   ...
   else if((last_frame_copy.imgY!=NULL) && (last_frame_copy.imgUV!=NULL) )
   {
      /*normal*/
   }
   else
   {
      ...
      printf("Error buffers are partially allocated\n");
   }

   for(i=0; i<p_Vid->height; i++)
   {
      memcpy(&(last_frame_copy.imgY[i][0]), &(dec_picture->imgY[i][0]), p_Vid->width*sizeof(imgpel));
   }

   for(i=0; i<p_Vid->height/2; i++)
   {
      memcpy(&(last_frame_copy.imgUV[0][i][0]), &(dec_picture->imgUV[0][i][0]), p_Vid->width*sizeof(imgpel)/2);
      memcpy(&(last_frame_copy.imgUV[1][i][0]), &(dec_picture->imgUV[1][i][0]), p_Vid->width*sizeof(imgpel)/2);
   }
#if SAVE_LAST_FRAME
   for(i=0; i<p_Vid->height; i++)
   {
      ...
      {
         fwrite(&(dec_picture->imgY[i][j]), 1, 1, outFile);
      }
   }
   for(i=0; i<p_Vid->height/2; i++)
   {
      for(j=0; j<p_Vid->width/2; j++)
      {
         fwrite(&(dec_picture->imgUV[0][i][j]), 1, 1, outFile);
      }
   }
   for(i=0; i<p_Vid->height/2; i++)
   {
      for(j=0; j<p_Vid->width/2; j++)
      {
         fwrite(&(dec_picture->imgUV[1][i][j]), 1, 1, outFile);
      }
   }
#endif
}
...
void exit_picture(VideoParameters *p_Vid, StorablePicture **dec_picture)
{
  ...
  if (*dec_picture==NULL || (p_Vid->num_dec_mb==0/*p_Vid->num_dec_mb != p_Vid->PicSizeInMbs*/ && (p_Vid->yuv_format != YUV444 || !p_Vid->separate_colour_plane_flag)))
  {
    return;
  }
...
#if WANT_SLICE_DROP
  copyToLastFrame(p_Vid, *dec_picture);
#endif
